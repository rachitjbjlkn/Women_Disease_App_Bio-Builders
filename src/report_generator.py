...
class PDFReportGenerator:
    ...
    def generate_pdf(self, patient_report, output_path, include_images=True):
        ...
        # Add images if available — all on ONE page, side by side, no empty pages
        if include_images and patient_report.ultrasound_images:
            # Filter to only existing images first
            valid_images = [img for img in patient_report.ultrasound_images
                            if os.path.exists(img['path'])]

            if valid_images:
                story.append(PageBreak())
                story.append(Paragraph("ULTRASOUND IMAGES", self.styles['SectionHeading']))
                story.append(Spacer(1, 0.15 * inch))

                if len(valid_images) >= 2:
                    # Side-by-side layout in a Table (no extra blank pages)
                    try:
                        left_img  = RLImage(valid_images[0]['path'], width=3.1*inch, height=2.7*inch)
                        right_img = RLImage(valid_images[1]['path'], width=3.1*inch, height=2.7*inch)

                        img_table = Table(
                            [[Paragraph(valid_images[0]['type'], self.styles['Normal']),
                              Paragraph(valid_images[1]['type'], self.styles['Normal'])],
                             [left_img, right_img]],
                        ...
                        img_table.setStyle(TableStyle([
                            ...
                            ('FONTNAME',    (0, 0), (-1, 0),  'Helvetica-Bold'),
                            ('FONTSIZE',    (0, 0), (-1, 0),  9),
                            ('TEXTCOLOR',   (0, 0), (-1, 0),  colors.HexColor('#2c5aa0')),
                        ...
                        for img_data in valid_images:
                            try:
                                story.append(Paragraph(img_data['type'], self.styles['Normal']))
                                story.append(RLImage(img_data['path'], width=5.5*inch, height=4*inch))
                                story.append(Spacer(1, 0.2*inch))
                            except Exception:
                                pass
                else:
                    # Only one image — just embed it, no extra page
                    try:
                        story.append(Paragraph(valid_images[0]['type'], self.styles['Normal']))
                        story.append(RLImage(valid_images[0]['path'], width=5.5*inch, height=4.5*inch))
                    except Exception:
                        pass
